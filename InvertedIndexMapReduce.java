...
public final class InvertedIndexMapReduce {
	...
	private String docID = "";
        ...
        protected void map(LongWritable key, Text value, Context context)
            ...
            for (String token : StringUtils.split(val)) {
		if (token.equals(docID))
			continue;
                word.set(token);
    ...
    public static class Reduce extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values,
            ...
            for (Text docId : values) {
                ...
                if (docCount.containsKey(docId.toString()))
                {
                    docCount.put(docId.toString(),docCount.get(docId.toString())+1);
                }
                else
                {
                    docCount.put(docId.toString(),1);
                }
            }
            String temp = "";
	    for (String tempkey : docCount.keySet())
            {
                temp += tempkey + ":"+ Integer.toString(docCount.get(tempkey)) + "\t";
            }
            docIds.set(new Text(temp));
